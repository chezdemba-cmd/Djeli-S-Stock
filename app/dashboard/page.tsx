...
import {
  ...
  WifiOff, Wifi, RefreshCw, Settings, UserPlus, Truck
} from "lucide-react";
import { processSale, createCustomer, createStore, createClientWorkspace, createProduct, addStockMovement, payReceivable, createEmployee, createSupplier, paySupplier } from "../../lib/db/business"; // Server Actions
...
type Supplier = { id: string; name: string; phone: string; balance: number; status: "À régler" | "À jour"; };
...
export default function Home() {
  ...
  const [modal, setModal] = useState<"product" | "movement" | "customer" | "supplier" | "depot" | "sale" | "receipt" | "new_client" | "inflow" | "payment" | "pay_supplier" | "new_employee" | null>(null);
    ...
    const checkAuthAndLoadData = async () => {
      ...
      if (online) {
        ...
        if (currentActiveOrgId) {
          ...
          if (supsData) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const mappedSuppliers: Supplier[] = supsData.map((s: any) => {
...
function SupplierTable({ suppliers, onPay }: { suppliers: Supplier[], onPay: (s: Supplier) => void }) {
  if (suppliers.length === 0) return <div style={{ padding: '2rem', textAlign: 'center', color: '#666' }}>Aucun fournisseur enregistré.</div>;
  return <div className="table-wrap"><table><thead><tr><th>Fournisseur</th><th>Contact</th><th>Dette (À payer)</th><th>Statut</th><th>Action</th></tr></thead><tbody>{suppliers.map((s) => <tr key={s.id}><td><strong>{s.name}</strong></td><td>{s.phone}</td><td><strong style={{ color: s.balance > 0 ? '#d32f2f' : 'inherit' }}>{money.format(s.balance)}</strong></td><td><span className={`status ${s.balance > 0 ? 'danger' : 'ok'}`}>{s.balance > 0 ? s.status : 'À jour'}</span></td><td>{s.balance > 0 ? <button className="button-secondary" style={{ padding: '0.3rem 0.6rem', fontSize: '0.85rem' }} onClick={() => onPay(s)}>💸 Payer</button> : <span style={{ color: '#888', fontSize: '0.8rem' }}>-</span>}</td></tr>)}</tbody></table></div>;
}
...
function StockInflowForm({ products, suppliers, onClose, onSubmit, isSubmitting, errorMsg }: { products: Product[], suppliers: Supplier[], onClose: () => void, onSubmit: (e: FormEvent<HTMLFormElement>) => void, isSubmitting: boolean, errorMsg: string | null }) {
  const [selectedProduct, setSelectedProduct] = useState("");
  const [qty, setQty] = useState(1);
    ...
    <>
      ...
      <form onSubmit={onSubmit}>
        <label className="wide">Produit livré
          <select name="product_id" required value={selectedProduct} onChange={(e) => setSelectedProduct(e.target.value)}>
            <option value="" disabled>Sélectionner un produit...</option>
            {products.map((p) => <option key={p.id} value={p.id}>{p.name} (Stock actuel: {p.quantity})</option>)}
          </select>
        </label>
        <label className="wide">Quantité reçue
          <input required type="number" min="1" name="quantity" value={qty} onChange={(e) => setQty(Number(e.target.value))} />
        </label>

        <label className="wide" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 'normal', cursor: 'pointer', marginTop: '1rem' }}>
          <input type="checkbox" checked={hasSupplier} onChange={(e) => setHasSupplier(e.target.checked)} /> Lier à un fournisseur (Achat)
        </label>

        {hasSupplier && (
          <div style={{ background: '#f5f5f5', padding: '1rem', borderRadius: '8px', marginTop: '0.5rem' }}>
            <label className="wide">Fournisseur
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }} className="wide">
              <label>Montant total de la facture (FCFA)
                <input type="number" min="0" name="payable_amount" required value={payableAmount} onChange={(e) => {
                  const val = Number(e.target.value);
                  setPayableAmount(val);
                  if (amountPaid > val) setAmountPaid(val);
                }} />
              </label>
              <label>Montant payé immédiatement (FCFA)
                <input type="number" min="0" max={payableAmount} name="amount_paid" required value={amountPaid} onChange={(e) => setAmountPaid(Number(e.target.value))} />
              </label>
            </div>
            {amountPaid < payableAmount && <p style={{ color: '#e65100', fontSize: '0.85rem', marginTop: '0.5rem' }}>Reste à payer : {money.format(payableAmount - amountPaid)} (Sera ajouté à la dette fournisseur)</p>}
          </div>
        )}
  ...
  return (
    <>
      <div className="modal-heading dont-print">
        <div className="modal-symbol"><ShoppingCart /></div>
        <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
          <div><h2>Reçu</h2><p>Imprimer ou Partager</p></div>
        </div>
      </div>
      
      <pre className="dont-print" style={{ background: '#f5f5f5', padding: '15px', borderRadius: '8px', fontSize: '13px', whiteSpace: 'pre-wrap', marginBottom: '1rem' }}>{text}</pre>
      ...
      <div className="printable-receipt">
        ...
        <div className="receipt-totals">
          <div className="receipt-row bold"><span>Total :</span><span>{money.format(receipt.total)}</span></div>
          <div className="receipt-row"><span>Payé :</span><span>{money.format(receipt.paid)}</span></div>
          <div className="receipt-row highlight"><span>Reste à Payer :</span><span>{money.format(receipt.due)}</span></div>
        </div>
      ...
      <div className="form-actions wide dont-print" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        <button type="button" onClick={() => handlePrint('ticket')} style={{ background: '#e7f1ed', color: '#173f35', flex: 1, padding: '10px 5px', fontSize: '12px' }}>🖨️ Ticket 80mm</button>
        <button type="button" onClick={() => handlePrint('a4')} style={{ background: '#e7f1ed', color: '#173f35', flex: 1, padding: '10px 5px', fontSize: '12px' }}>🖨️ Facture A4</button>
        <a href={url} target="_blank" rel="noreferrer" className="button primary" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', flex: 1, padding: '10px 5px', fontSize: '12px' }}>📱 WhatsApp</a>
      </div>
    </>
  );
}
...
function SupplierForm({ onClose, onSubmit, isSubmitting, errorMsg }: { onClose: () => void, onSubmit: (e: FormEvent<HTMLFormElement>) => void, isSubmitting: boolean, errorMsg: string | null }) {
  return <>
    <div className="modal-heading"><div className="modal-symbol"><Truck /></div><div><h2>Nouveau Fournisseur</h2><p>Ajouter un fournisseur pour vos arrivages.</p></div></div>
    {errorMsg && <div className="alert-error" style={{ color: 'red', marginBottom: '1rem', background: '#ffebee', padding: '10px', borderRadius: '8px' }}>{errorMsg}</div>}
    <form onSubmit={onSubmit}>
      <label className="wide">Nom du fournisseur <input required type="text" name="name" placeholder="Ex: Grossiste ABC" /></label>
      <label className="wide">Téléphone <input type="tel" name="phone" placeholder="Ex: 01 23 45 67 89" /></label>
      <div className="form-actions wide" style={{ marginTop: '1rem' }}>
        <button type="button" onClick={onClose}>Annuler</button>
        <button className="primary" type="submit" disabled={isSubmitting}>{isSubmitting ? "Création..." : "Ajouter le fournisseur"}</button>
      </div>
    </form>
  </>;
}

function PaySupplierForm({ supplier, onClose, onSubmit, isSubmitting, errorMsg, money }: { supplier: Supplier, onClose: () => void, onSubmit: (e: FormEvent<HTMLFormElement>) => void, isSubmitting: boolean, errorMsg: string | null, money: Intl.NumberFormat }) {
  return (
    <>
      <div className="modal-heading">
        <div className="modal-symbol"><CircleDollarSign /></div>
        <div><h2>Règlement Fournisseur</h2><p>Paiement pour {supplier.name}</p></div>
      </div>
      {errorMsg && <div className="alert-error" style={{ color: 'red', marginBottom: '1rem', background: '#ffebee', padding: '10px', borderRadius: '8px' }}>{errorMsg}</div>}
      <div style={{ background: '#fff8e1', borderLeft: '4px solid #ffa000', padding: '12px', borderRadius: '4px', marginBottom: '1rem' }}>
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="form-actions wide" style={{ marginTop: '1rem' }}>
          <button type="button" onClick={onClose}>Annuler</button>
          <button className="primary" type="submit" disabled={isSubmitting}>{isSubmitting ? "Validation..." : "Enregistrer le décaissement"}</button>
        </div>
      </form>
    </>
  );
...
function EmployeeTable({ employees }: { employees: any[] }) {
  if (employees.length === 0) return <div style={{ padding: '2rem', textAlign: 'center', color: '#666' }}>Aucun employé enregistré.</div>;
  return <div className="table-wrap"><table><thead><tr><th>Nom</th><th>Rôle</th><th>Dépôt assigné</th><th>Date d&apos;ajout</th></tr></thead><tbody>{employees.map((emp) => <tr key={emp.membership_id}><td><strong>{emp.full_name}</strong></td><td>{emp.role === 'owner' ? 'Propriétaire' : emp.role === 'manager' ? 'Gérant' : 'Caissier / Vendeur'}</td><td>{emp.store_name || '-'}</td><td>{new Date(emp.created_at).toLocaleDateString('fr-FR')}</td></tr>)}</tbody></table></div>;
}

function EmployeeForm({ depots, onClose, onSubmit, isSubmitting, errorMsg }: { depots: Depot[], onClose: () => void, onSubmit: (e: FormEvent<HTMLFormElement>) => void, isSubmitting: boolean, errorMsg: string | null }) {
  return (
    <>
      <div className="modal-heading">
        <div className="modal-symbol"><UserPlus /></div>
        <div><h2>Ajouter un employé</h2><p>Le mot de passe initial sera requis pour sa connexion.</p></div>
      </div>
      {errorMsg && <div className="alert-error" style={{ color: 'red', marginBottom: '1rem', background: '#ffebee', padding: '10px', borderRadius: '8px' }}>{errorMsg}</div>}
      <form onSubmit={onSubmit}>
        <label className="wide">Nom complet <input name="full_name" required type="text" placeholder="Ex: Aminata Traoré" /></label>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }} className="wide">
          <label>Adresse e-mail <input name="email" required type="email" placeholder="aminata@exemple.com" /></label>
          <label>Mot de passe initial <input name="password" required type="password" minLength={6} placeholder="••••••••" /></label>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }} className="wide">
          <label>Rôle
        ...
        <div className="form-actions wide" style={{ marginTop: '1rem' }}>
          <button type="button" onClick={onClose}>Annuler</button>
          <button className="primary" type="submit" disabled={isSubmitting}>{isSubmitting ? "Création..." : "Ajouter l'employé"}</button>
        </div>
      </form>
    </>
  );
